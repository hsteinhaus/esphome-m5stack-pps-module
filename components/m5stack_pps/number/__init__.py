...
from esphome.const import CONF_ID, CONF_CHANNEL, CONF_VOLTAGE, CONF_CURRENT, DEVICE_CLASS_VOLTAGE, DEVICE_CLASS_CURRENT, ENTITY_CATEGORY_CONFIG
...
async def to_code(config):
    ...
    if output_channel == CONF_VOLTAGE:
        await number.register_number(var, config, min_value=0.5, max_value=30, step=0.1)
        cg.add(var.set_parent(paren))
        cg.add(paren.set_output_voltage_number(var))
    else:
        await number.register_number(var, config, min_value=0, max_value=5, step=0.001)
        cg.add(var.set_parent(paren))
